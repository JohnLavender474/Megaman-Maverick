...
class RedKoopaShell(game: MegamanMaverickGame) : AbstractEnemy(game), IFaceable {

    companion object {
        const val TAG = "RedKoopaShell"
    ...
    private enum class RedKoopaShellState {
    ...
    private fun startRolling() {
        state = RedKoopaShellState.ROLLING
    ...
    private fun getKnockedToDeath(shellBounds: GameRectangle) {
        state = RedKoopaShellState.KNOCKED_TO_DEATH
    ...
    override fun defineBodyComponent(): BodyComponent {
        ...
        headFixture.setHitByFeetReceiver(ProcessState.BEGIN) { feet, _ ->
            ...
            if (entity is Megaman) {
                if (state == RedKoopaShellState.IDLE) startRolling()
        ...
        body.preProcess.put(ConstKeys.DEATH) {
            body.physics.collisionOn = state != RedKoopaShellState.KNOCKED_TO_DEATH
            if (state == RedKoopaShellState.KNOCKED_TO_DEATH) body.forEachFixture { it.setActive(false) }
        }

        body.preProcess.put(ConstKeys.DAMAGER) {
            val damager = body.fixtures.get(FixtureType.DAMAGER).first()
            damager.setActive(state == RedKoopaShellState.ROLLING)
        }

        body.preProcess.put(ConstKeys.GRAVITY) {
            val gravity = when {
                !body.isSensing(BodySense.FEET_ON_GROUND) || state == RedKoopaShellState.KNOCKED_TO_DEATH -> GRAVITY
                else -> GROUND_GRAVITY
            }
        ...
        body.postProcess.put(ConstKeys.SCANNER) {
            if (state == RedKoopaShellState.KNOCKED_TO_DEATH) return@put
    ...
    override fun defineSpritesComponent() = SpritesComponentBuilder()
        ...
        .preProcess { _, sprite ->
            ...
            sprite.setFlip(isFacing(Facing.RIGHT), state == RedKoopaShellState.KNOCKED_TO_DEATH)
