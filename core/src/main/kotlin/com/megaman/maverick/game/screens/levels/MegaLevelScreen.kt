...
class MegaLevelScreen(private val game: MegamanMaverickGame) :
    ...
    override fun init(vararg params: Any) {
        ...
        cameraManagerForRooms.onFirstUpdateAfterReset = {
    ...
    override fun start(tmxMapSource: String) {
        ...
        val worldContainer: IWorldContainer

        when (game.params.worldContainer) {
            "simple" -> worldContainer = SimpleGridWorldContainer(ConstVals.PPM)
            "quadtree" -> {
                ...
                worldContainer = LooseQuadtreeWorldContainer(
                    ppm = tmr.tileWidth,
                    worldMinX = 0f,
                    worldMinY = 0f,
            ...
            else -> throw IllegalArgumentException("Invalid world container type: ${game.params.worldContainer}")
        }
    ...
    override fun onEvent(event: Event) {
        ...
        when (event.key) {
            ...
            EventType.PLAYER_SPAWN -> {
                ...
                val spawnProps =
                    playerSpawnsMan.currentSpawnProps ?: throw IllegalStateException("Megaman spawn props are null")
                ...
                if (cameraManagerForRooms.nextGameRoom()?.properties?.containsKey(ConstKeys.MUSIC) == true) {
                    val musicAsset = MusicAsset.valueOf(
                        cameraManagerForRooms
                            .currentGameRoom!!
                            .properties
                            .get(ConstKeys.MUSIC, String::class.java)
                            .uppercase()
                    )
    ...
    override fun render(delta: Float) {
        ...
        if (!game.paused) {
            ...
            // Because I'm not good at software design, there's some tight coupling in this block
    ...
    private fun tryToPlayCurrentRoomMusic() {
        if (cameraManagerForRooms.currentGameRoom?.properties?.containsKey(ConstKeys.MUSIC) == true) {
            val musicAsset = MusicAsset.valueOf(
                cameraManagerForRooms
                    .currentGameRoom!!
                    .properties
            ...
            if (musicAsset != audioMan.getCurrentMusicAsset()) musicAsset.let { audioMan.playMusic(it, it.loop) }
        }
    }
}
