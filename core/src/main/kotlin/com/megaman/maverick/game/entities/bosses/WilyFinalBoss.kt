...
class WilyFinalBoss(game: MegamanMaverickGame) : AbstractBoss(game), IAnimatedEntity, IFreezableEntity {
    ...
    override fun onSpawn(spawnProps: Properties) {
        ...
        val planeBodyTarget = spawnProps
            .get("skull_head_target", RectangleMapObject::class)!!
            .rectangle.getCenter()
        phaseTransitionHandler.planeBodyTarget.set(planeBodyTarget)
    ...
    private inner class Phase1Handler : PhaseHandler {
        ...
        override fun buildBody(body: Body) {
            ...
            body.preProcess.put(ConstKeys.DEFAULT) {
                ...
                val hovering = phaseTransitionHandler.active ||
                    (state == WilyPhase1State.FLY_IN && phase1Handler.flyInDecelerating) ||
                    state.equalsAny(
    ...
    private object Phase2ConstVals {
        ...
        const val CANNON_FIRE_INTERVAL_HARD = 4f
    ...
    private inner class Phase2Handler : PhaseHandler, Updatable {
        ...
        private fun launchOrbs() {
            ...
            leftOrb?.let { orb ->
                ...
                if (dist > 0.0001f) {
                    orb.body.physics.velocity.set(
                        dx / dist * orbLaunchSpeed * ConstVals.PPM,
                        dy / dist * orbLaunchSpeed * ConstVals.PPM
                    )
                }
                leftOrb?.putCullable(
    ...
    private inner class Phase3Handler : PhaseHandler {
        ...
        fun getSpriteAlpha(): Float {
            ...
            val rawAlpha = when (sm.getCurrentElement()) {
                ...
                WilyPhase3State.APPEAR -> fadeTimer.getRatio()
                WilyPhase3State.VANISH -> 1f - fadeTimer.getRatio()
            }
        ...
        private fun onChangeState(current: WilyPhase3State, previous: WilyPhase3State) {
            ...
            when (current) {
                ...
                WilyPhase3State.APPEAR -> {
                    ...
                    fadeTimer.resetDuration(Phase3ConstVals.FADE_DUR)
                }
        ...
        override fun init(vararg params: Any) {
            ...
            val fadeDur = if (game.state.hardMode) Phase3ConstVals.FADE_DUR_HARD else Phase3ConstVals.FADE_DUR
            fadeTimer.resetDuration(fadeDur)
    ...
    private inner class PhaseTransitionHandler : Updatable, Resettable {
        ...
        private fun spawnPlaneBody() {
            GameLogger.debug(TAG, "spawnPlaneBody()")
            val planeBody = MegaEntityFactory.fetch(WilyPlaneBody::class)!!
            planeBody.spawn(
                props(
                    ConstKeys.TARGET pairTo planeBodyTarget,
