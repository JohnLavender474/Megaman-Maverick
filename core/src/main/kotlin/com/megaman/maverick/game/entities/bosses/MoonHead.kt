...
class MoonHead(game: MegamanMaverickGame) : AbstractBoss(game, dmgDuration = DAMAGE_DUR, size = Size.LARGE),
    ...
    override fun onSpawn(spawnProps: Properties) {
        ...
        if (spawnProps.containsKey(AsteroidsSpawner.TAG)) {
            val spawnerObj = spawnProps.get(AsteroidsSpawner.TAG, RectangleMapObject::class)!!
            asteroidsSpawner = MegaEntityFactory.fetch(AsteroidsSpawner::class)!!
            asteroidsSpawner!!.spawn(
                props(
                    ConstKeys.SPAWNER pairTo spawnerObj,
    ...
    private fun shoot() {
        ...
        val speed = if (game.state.getDifficultyMode() == DifficultyMode.HARD)
            SHOOT_SPEED_HARD else SHOOT_SPEED

        val impulse = megaman.body.getCenter().sub(spawn).nor().scl(speed * ConstVals.PPM)
