...
class LaserBeamerV2(game: MegamanMaverickGame) : MegaGameEntity(game), ISpritesEntity, IBodyEntity,
    ICullableEntity, IAudioEntity, IEventListener {

    companion object {
        const val TAG = "LaserBeamerV2"
    ...
    override fun init() {
        ...
        if (region == null) region = game.assMan.getTextureRegion(TextureAsset.HAZARDS_1.source, LaserBeamer.TAG)
        super.init()
    ...
    override fun onSpawn(spawnProps: Properties) {
        ...
        spawnProps.forEach { key, value ->
            if (key.toString().contains(ConstKeys.BLOCK)) laserProps.put(key, value)
        }
