...
class SimpleGridWorldContainerTest : DescribeSpec({
    describe("SimpleGridWorldContainer") {
        ...
        it("filter should exclude bodies that do not pass the predicate") {
            ...
            // When – only keep DYNAMIC bodies
            grid.getBodies(1, 1, outBodies1) { it.type == BodyType.DYNAMIC }

            // Then
            outBodies1 shouldContain body1
            outBodies1 shouldNotContain body2
        }

        it("filter should exclude bodies in a range query that do not pass the predicate") {
            // Given
            val body1 = Body(BodyType.DYNAMIC, 10f, 10f, 10f, 10f)
            val body2 = Body(BodyType.STATIC, 20f, 20f, 10f, 10f)
            grid.addBody(body1)
            grid.addBody(body2)

            // When – only keep STATIC bodies
            grid.getBodies(1, 1, 3, 3, outBodies1) { it.type == BodyType.STATIC }

            // Then
            outBodies1 shouldNotContain body1
            outBodies1 shouldContain body2
        }

        it("filter should exclude fixtures that do not pass the predicate") {
            // Given
            val body = mockk<Body>()
            val fixture1 = Fixture(body, "TypeA", GameRectangle(10f, 10f, 10f, 10f), attachedToBody = false)
            val fixture2 = Fixture(body, "TypeB", GameRectangle(10f, 10f, 10f, 10f), attachedToBody = false)
            grid.addFixture(fixture1)
            grid.addFixture(fixture2)

            // When – only keep fixtures of type "TypeA"
            grid.getFixtures(1, 1, outFixtures) { it.getType() == "TypeA" }
        ...
        it("filter should exclude fixtures in a range query that do not pass the predicate") {
            // Given
            val body = mockk<Body>()
            val fixture1 = Fixture(body, "TypeA", GameRectangle(10f, 10f, 10f, 10f), attachedToBody = false)
            val fixture2 = Fixture(body, "TypeB", GameRectangle(20f, 20f, 10f, 10f), attachedToBody = false)
            grid.addFixture(fixture1)
            grid.addFixture(fixture2)

            // When – only keep fixtures of type "TypeB"
            grid.getFixtures(1, 1, 3, 3, outFixtures) { it.getType() == "TypeB" }
